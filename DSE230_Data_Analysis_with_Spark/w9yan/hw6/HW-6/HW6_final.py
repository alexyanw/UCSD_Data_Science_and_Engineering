...
# Your code here
input_list = open('../../Data/hw6-files-4gb.txt').readlines()
files = map(lambda f: f.rstrip(), input_list)
...
def safe_parse(raw_json):
    ...
    try: 
        ...
        #if 'limit' in json_object: return 0
        return json_object
    except: 
        return 0

rdd1 = rdd.map(safe_parse).filter(lambda x:x!=0 and 'limit' not in x)
valid_rdd = rdd1.map(lambda x: (x['user']['id_str'], x['text'])).cache()
...
# your code here
count_user = valid_rdd.groupByKey().count()
print_users_count(count_user)
...
user_partition = pickle.load(open(filename,'rb'))
...
# your code here
#group_user_rdd = valid_rdd.map(lambda x: (user_partition.get(x[0], 7),x))
#group_count_rdd = group_user_rdd.groupByKey().mapValues(len)


# In[109]:

# method 2 - try joining
partition_rdd = sc.parallelize(user_partition.items())
group_user_rdd1 = valid_rdd.leftOuterJoin(partition_rdd)
group_count_rdd = group_user_rdd1.map(lambda x: (7 if x[1][1] is None else x[1][1],0)).groupByKey().mapValues(len)
#
#print_post_count(sorted(group_count_rdd1.collect()))
...
# your code here
print_post_count(sorted(group_count_rdd.collect()))
...
token_rdd = valid_rdd.map(lambda x: {'user_id': x[0], 'token': tok.tokenize(x[1].encode('utf-8'))}).cache()
token_count_rdd = token_rdd.flatMap(lambda x: dict(Counter(x['token'])).items())
token_sum_rdd = token_count_rdd.groupByKey()
print_count(token_sum_rdd)
...
# your code here
token_user_rdd = token_rdd.flatMap(lambda x: [(t,x['user_id']) for t in x['token']])
token_users_rdd = token_user_rdd.groupByKey().mapValues(lambda users: set(users)).filter(lambda x: len(x[1]) > 99).cache()
token_count_user_rdd = token_users_rdd.map(lambda x: (x[0],len(x[1])))


# In[117]:

print_count(token_count_user_rdd)
print_tokens(token_count_user_rdd.top(20, lambda x:x[1]))
...
# your code here
def get_partition_vector(users):
    return [user_partition.get(u, 7) for u in users]

token_partition = token_users_rdd.mapValues(get_partition_vector)


# In[119]:

# TRY
def get_partition_count(partitions):
    #count_partition = [0 for i in range(8)]
    #for p in partitions:
    #    count_partition[p] += 1
    #return count_partition
    count_partition = {i:0 for i in range(8)}
    count_partition.update(dict(Counter(partitions)).items())
    return count_partition.values()
        
token_partition_count = token_partition.mapValues(get_partition_count)


# In[ ]:

def compute_popularity(partition_count):
    from math import log
    total = sum(partition_count)
    return map(lambda c: get_rel_popularity(c, total) if c > 0 else -10000000, partition_count)
        
token_popularity = token_partition_count.mapValues(compute_popularity).cache()


# In[ ]:

for group in range(8):
    group_popularity = token_popularity.map(lambda x: (x[0], x[1][group])).sortBy(lambda x: (-x[1], x[0]))
    print_tokens(group_popularity.take(10), group)
